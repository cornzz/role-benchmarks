...
def plot_data(labels, means, errors, appr, title):
    ...
    if len(appr) == 2:
        ...
        ax.set_ylabel('Run time factor normalized to Classic 2020')
    else:
        ax.bar(x=(x - w), height=h[0], width=w, yerr=yerr[0], label=appr[0], capsize=cs)
        ax.bar(x=x,       height=h[1], width=w, yerr=yerr[1], label=appr[1], capsize=cs)
        ax.bar(x=(x + w), height=h[2], width=w, yerr=yerr[2], label=appr[2], capsize=cs)
